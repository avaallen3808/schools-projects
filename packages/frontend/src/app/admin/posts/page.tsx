  ...
  return (
    <section className="px-8 py-10">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => setIsOpen(true)} className="px-5 py-2 text-sm font-medium bg-primary text-text rounded-pill border-none cursor-pointer">+ Tambah Post</button>
      </div>

      <DataTable
        columns={[
          { key: 'title', header: 'Judul', render: (p: any) => <span className="font-medium">{p.title}</span> },
          { key: 'category', header: 'Kategori', render: (p: any) => p.category?.name || <span className="text-xs text-text-secondary">-</span> },
          { key: 'published', header: 'Status', render: (p: any) => <Badge label={p.published ? 'published' : 'draft'} /> },
          { key: 'date', header: 'Tanggal', render: (p: any) => <span className="text-xs text-text-secondary">{new Date(p.createdAt).toLocaleDateString('id-ID')}</span> },
          { key: 'actions', header: '', render: (p: any) => (
            <button onClick={() => deleteMutation.mutate(p.id)} className="text-xs underline text-error bg-transparent border-none cursor-pointer">Hapus</button>
          )},
        ]}
        data={postsData?.data || []}
