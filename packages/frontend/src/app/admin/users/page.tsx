  ...
  return (
    <section className="px-8 py-10">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl" style={{ fontFamily: 'Prata, serif' }}>Users</h1>
        <button onClick={() => setIsOpen(true)} className="px-5 py-2 text-sm font-medium"
          style={{ background: '#fdc72f', color: '#000', borderRadius: 100, border: 'none', cursor: 'pointer' }}>+ Tambah User</button>
      </div>

      <DataTable
        columns={[
          { key: 'name', header: 'Nama', render: (u: any) => u.name },
          { key: 'email', header: 'Email', render: (u: any) => <span className="text-xs" style={{ color: '#666' }}>{u.email}</span> },
          ...
          { key: 'createdAt', header: 'Dibuat', render: (u: any) => <span className="text-xs" style={{ color: '#999' }}>{new Date(u.createdAt).toLocaleDateString('id-ID')}</span> },
          { key: 'actions', header: '', render: (u: any) => (
            <button onClick={() => deleteMutation.mutate(u.id)} className="text-xs underline" style={{ color: '#dc2626', background: 'none', border: 'none', cursor: 'pointer' }}>Hapus</button>
          )},
        ]}
        data={usersData?.data || []}
      />
