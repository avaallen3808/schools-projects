...
export function Badge({ label }: { label: string }) {
  const style = variants[label] || 'background: #f3f4f6; color: #374151;';
  return (
    <span className="inline-block px-2 py-0.5 rounded-full text-xs font-medium" style={{ ...parseStyle(style) }}>
      {label}
    </span>
  );
}
...
function parseStyle(s: string): React.CSSProperties {
  return Object.fromEntries(s.split(';').filter(Boolean).map((p) => p.split(':').map((x) => x.trim())));
}
